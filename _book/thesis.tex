...
\author{Joseph A. Student} % {Joseph A. Student}
\degree{Doctor of Philosophy} % {Master of Science}
\doctype{Dissertation}
\submitdate{January 1, 2013} % {January 1, 2013}
\commencementyear{2013} % {2013}

\department{Nuclear Engineering and Radiation Health Physics} % {Nuclear Engineering and Radiation Health Physics}

\depttype{School} % {School}

\depthead{Head} % {Director}

\major{Radiation Health Physics} % {Radiation Health Physics}

\advisor{Jane R. Professor} % {Jane R. Professor}
...
\mainmatter
  ...
  \begin{Shaded}
  \begin{Highlighting}[]
  \KeywordTok{include_graphics}\NormalTok{(}\DataTypeTok{path =} \StringTok{"figure/frog.jpg"}\NormalTok{)}
  \end{Highlighting}
  \end{Shaded}
  
  \begin{figure}
  
  {\centering \includegraphics[width=9cm]{figure/frog} 
  
  }
  
  \caption[Frogs are weird..]{Frogs are weird...}\label{fig:reedlogo}
  \end{figure}
  ...
  \NormalTok{if(!}\KeywordTok{require}\NormalTok{(thesisdown))}
    \NormalTok{devtools::}\KeywordTok{install_github}\NormalTok{(}\StringTok{"zkamvar/thesisdown@osu-thesis"}\NormalTok{)}
  \KeywordTok{library}\NormalTok{(thesisdown)}
  \end{Highlighting}
  \end{Shaded}
