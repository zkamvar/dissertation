...
sexual reproduction and different levels of population mixture in both
microsatellite and SNP data sets, (2) perform a power analysis of $\bar{r}_d$ to
assess sensitivity and specificity, (3) assess how genotypic and allelic
...
Because of a pattern that we saw in the results of $E_{5A}$, we decided to ask
the question of whether or not using a value of $E_{5A} \geq 0.85$ could help
detect clonal reproduction even if the p-value was non-significant. To assess
this, we parameterized an additional ROC analysis to "flip" any non-significant
value if it met the above criteria.
...
low sample sizes (Fig. \@ref(fig:sim4a)). Analyzing the distributions of the
AURC with AMOVA, we found that the most consistently significant effect found
was that of sample size (Fig. \@ref(fig:sim3a), Tab. \@ref(tab:sim4)) (albiet
much like Fig. \@ref(fig:sim3), all effects and interactions were significant
for 0.1% and 0.5% sex). 
...
\times Sample Size \times Mutation Rate$ for each rate of sexual reproduction
separately. This table generated with the allelic evenness correction. 
CC = Clone Correction, SS = Sample Size, MR = Mutation Rate.
...
each rate of sexual reproduction separately with SNP data. SS = Sample Size.
