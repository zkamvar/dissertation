...
created with a census size of 10,000 individuals with equal mating type proportions evolved over 10,000
...
with the `RandomSelection()` method and offspring genotypes were created via the
`MendelianGenoTransmitter()` method. The clonal fraction was created by randomly
...
alleles with frequencies drawn from a uniform distribution and subsequently normalized.
Before mating, mutations occurred at each locus at a rate of 1e-5
mutations/generation in a stepwise manner using the `StepwiseMutator()` method. 
