...
organisms are managed by antimicrobial compounds that kill these organisms. 
...
lead to resistant genotypes [@smith1993how; @milgroom1996recombination;
...
The Receiver Operating Characteristic (ROC) curve is a statistical method of assessing the
...
blue marbles through. The fraction of orange and blue marbles that passed 
...
microsatellite and SNP data. Additionally,
with larger sample sizes, the variance also decreases, suggesting that the behavior of the
index is also affected by sample size.
...
shows consistent differences in rate of sexual reproduction and sample size. 
...
Each violin plot contains 1000 unique data sets. Black lines in violins mark the 25, 50,
and 75th percentile."
...
cap <- "Genotypic and allelic diversity statistics shown over increasing rates
of sexual reproduction. Genotypic diversity: H = Shannon-Wiener index, $E_5$ =
evenness, G = Stoddart and Taylor's index. Allelic Diversity: $E_{5A}$ = allelic
evenness, $H_{exp}$ = Nei's expected heterozygosity (gene diversity). Points 
indicate means and lines extend to to standard deviations on either side of the 
mean. Color/shading indicates sample size. Vertical axis on different scales."

figscapdiv <- "Genotypic and allelic diversity statistics shown over increasing
...
figcap3 <- "Power (true positive rate) to detect non-random mating with changes
in rates of sexual reproduction in SSR data. Plots are arranged in a grid with
...
figcaproc <- "ROC curves for $\\bar{r}_d$ over each rate of sexual reproduction
(rows) in clone corrected (cc) and whole (wd) data sets with even and uneven
mutation rates (columns). Line type indicates the use of $E_{5A}$ > 0.85 to
augment the analysis. Line shade indicates sample size. A dotted line of unity
is shown in each plot. Each ROC curve is calculated over 100 values of $\\alpha$
from 0 to 1."

figcaproc  <- beaverdown::render_caption(caption = figcaproc, figname = "fig1", to = TO)
figscaproc <- "ROC curves for $\\bar{r}_d$."
...
correction consistently appears to reduce the AURC. For all
scenarios, the diagnostic power of $\bar{r}_d$ to detect non-random mating is no
better than random at 10% or greater rates of sexual reproduction.
...
values below 0.5 (shaded in grey) are considered worse than random."
...
### Low Allelic Diversity Creates Negative Values of $\bar{r}_d$ in Clonal Populations

When using the criteria of $E_{5A} \geq 0.85$ to augment the criteria for the 
...
distributions of the AURC with AMOVA, we found that the most consistently
significant effect found was that of sample size (Fig. \@ref(fig:sim3a), Tab.
\@ref(tab:sim4)) (albiet much like Fig. \@ref(fig:sim3), all effects and
interactions were significant for 0.1% and 0.5% sex).
...
with increasing rates of sexual reproduction in SSR data taking into account
allelic evenness. Plots are arranged in a grid with clone-correction in rows and
mutation rate in columns. Values are shown for $\\alpha$ = 0.01 and $E_{5A}
\\geq 0.85$"
figcap3  <- beaverdown::render_caption(caption = figcap3, figname = "fig1", to = TO)
figscap3 <- "Power (true positive rate) to detect non-random mating decreasing
with increasing rates of sexual reproduction in SSR data taking into account 
...
figcap2a <- "Box plots showing the distribution of the area under the ROC Curve
for each independent seed taking into account allelic evenness. Boxes span the 
interquartile range (IQR) and the whiskers extend to 1.5 * IQR. An AURC of 1
indicates a perfect classifier, while an AURC of 0.5 indicates the classifier is
no better than a random guess. Any values below 0.5 (shaded in grey) is
considered worse than random."

figscap2a <- "Box plots showing the distribution of the area under the ROC Curve for 
each independent seed taking into account allelic evenness."
```
...
figcap3a <- "Heatmap representing p-values from ANOVA analysis on AURC
distributions with the model $AURC \\sim Clone Correction \\times Sample Size 
\\times Mutation Rate$ for each rate of sexual reproduction separately. Allelic 
evenness was taken into account when calculating the ROC."
figcap3a  <- beaverdown::render_caption(caption = figcap3a, figname = "fig1", to = TO)
figscap3a <- "Heatmap representing p-values from ANOVA analysis on AURC
distributions taking into account allelic evenness."
```
...
Analysis of $\bar{r}_d$ for SNP data showed a similar pattern to microsatellite
...
values (Fig. \@ref(fig:sim5)). An interesting observation is that none of the
...
(\@ref(fig:simmisssnp)). This effect was exacerbated when missing data was
...
for all but two clonal data sets. After shuffling each chromosome independently,
we were able to obtain significance values that better reflected our
...
additional increase to false positive rates (Fig. \@ref(fig:sim6)). An AMOVA
...
in four different population sizes. Points represent the observed values. Black
lines mark the 25, 50, and 75th percentile."
...
cap <- "Violin plots showing the effect of increasing rates of missing data on
$\\bar{r}_d$ for SNP data. measured with declining rates of sexual reproduction.
Panels are arranged horizontally with increasing rates of missing data from left
to right and vertically with increasing sample size from top to bottom. Shading
indicates whether or not missing data was ignored or treated as a new allele.
Black lines mark the 25, 50, and 75th percentile."

scap <- "Violin plots showing the effect of increasing rates of missing data on
the $\\bar{r}_d$ for SNP data."
...
figcap6 <- "Power (true positive rate) to detect non-random mating decreasing with
...
transparency. Data shown for both p = 0.01 and p = 0.05"

figscap6 <- "Power to detect non-random mating decreasing with increasing rates of
sexual reproduction in SNP data."
```

```{r sim7, echo = FALSE, fig.cap = figcap7, fig.scap = figscap7}
knitr::include_graphics("figure/simulations/AURC_genomic.png")
figcap7 <- "Distributions of AURC for SNP data over increasing rates of sexual
reproduction Points represent AURC for 10 populations, 5 produced via random 
mating and 5 produced via non random mating."

figscap7 <- "Distributions of AURC for SNP data."
```
sexrate   term            sumsq   df   statistic   p.value
--------  ------------  -------  ---  ----------  --------
...
Knowing a pathogen's mode of reproduction can affect management strategies, and
a  it's
additionally important to know how sample size, rate of sexual reproduction,
mutation rate homogeneity, and clone-correction affect this method's sensitivity
and specificity.

Main points:

 - Permutation analyses of $\bar{r}_d$ appear to have sufficient power to detect
   clonal reproduction as long as the rate of sexual reproduction does not reach
   levels greater than 1%.
 - Power decreases with sample size
 - Clone correction decreases sample size and will decrease power. Interpret the
   results of clone-correction carefully.
 - A failure to reject the null hypothesis of random mating does not mean that
   the population is randomly mating [@smith1993how; @tibayrenc1996towards].
 - Ignoring linkage when doing permutation tests will show significant effects
   even if none exists.

Design Drawbacks:

 - Effect of uneven mutation rate might simply be an effect of a higher mutation
   rate.
 - Homoplasy would be prevalent with the few allelic states used to initialize
   the population.
