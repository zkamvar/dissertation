...
Population genetic theory is largely based on the neutral Wright-Fisher model 
in which populations are infinitely large, with discrete generations, 
no migration, no mutation, and randomly assorting alleles [@hartl1997principles;
@nielsen2013introduction]. By using this model, population genetics are able to
ask fundemental questions and test hypotheses about evolutionary processes that
could lead to structured populations. (((EXAMPLE OF COOL RESEARCH))).

This neutral model, however, cannot be applied to populations whose life history
...
applied to populations where the reproducive cycle is not partitioned into
...
this is the index of assocation ($I_A$), and its standardized version,
...
multimodal distribution of distances, and $I_A$ would be greater than zero
...
clone-correction, and the assumption of homogenous mutation rates affect our
...
additionally seek to deterimine what factors result in negative $\bar{r}_d$ 
values.
