...
association. It has been demonstrated that this index shows high variance with
low levels of sexual reproduction, but power analysis has not been performed. We
investigated the effect of sample size, mutation rate, and clone correction on 
the power of the index of association to detect clonal reproduction in simulated
data sets generated with microsatellite and genomic markers. Our findings
suggest that power decreases with small sample sizes and low allelic diversity,
and physical linkage in genomic markers does not affect power if permutation 
tests are performed with linkage groups. We hope that these novel insights 
provide useful to the study of molecular pathogens. 
...
resulting in non-significant p-values, which appear to be caused by low allelic
diversity. Additionally, with larger sample sizes, the variance also decreases,
suggesting that the behavior of the index is also affected by sample size. Based
...
Knowing a pathogen's mode of reproduction can affect the development of rational
management strategies. Researchers have used the index of association to provide
evidence for clonal reproduction in populations with support from permutation
analysis. @de2004clonal showed that very little sexual reproduction is required
for $\bar{r}_d$ to reach values close to zero. We confirmed that the power to
detect clonal reproduction is greatly diminished after $\sim$ 1% sex in a 
population. We provide here novel insights into how $\bar{r}_d$ responds to 
...
decreases with clone-correction.
...
uncorrected data in the hopes that this will reveal any underlying recombination
[@smith1993how; @mcdonald1997population]. When analyzing
...
failure to reject the null hypothesis of random mating does not mean that the 
population is randomly mating [@smith1993how; @tibayrenc1996towards]. 

### Mutation Rates and $\bar{r}_d$ 

We proposed to assess uneven mutation rate because we had observed that a
single, hypervarible locus with 18 alleles  was the driver for most of the
diversity observed in the 2001 outbreak of *Phytophthora ramorum* in Curry
County, OR (@kamvar2015spatial, Supplementary Table S4 (\@ref(tab:ramtabS4))).
...
that the opposite happened (Fig. \@ref(fig:sim4), \@ref(fig:simroc)). This also
...
A novel
feature that we found in our results not present in @de2004clonal was the 
presence of negative values of $\bar{r}_d$ (Fig. \@ref(fig:sim1)). While positive values of $\bar{r}_d$
...
evidence for random mating. We had previously observed significantly (p = 1) 
negative values of $\bar{r}_d$ of pathogens that are explicitly known to be
clonal such as the sudden oak death pathogen, *Phytophthora ramorum*
(@kamvar2015spatial, Supplementary Table S3 (Fig. \@ref(tab:ramtabS3))) and the wheat stripe rust pathogen
...
$E_{5A}$ > 0.9 and, on average, fewer than 4 alleles per locus. This is reflected
in our simulations (Fig. \@ref(fig:simdiv)). When we parameterized our power
analysis by considering a positive value as $\{p \leq \alpha \mid E_{5A} \geq 
0.85\}$ and found that this condition helped drastically reduce the number of 
...
also had low number of allelic states, which indicates that the bimodal
...
effect we see is not due to higher average mutation rate, thus further study is
...
reflect microsatellites that would be selected for population genetic study.
...
have seen in data from purely clonal organisms.
...
hypothesis of clonal reproduction in natural populations. We confirm the
findings of @de2004clonal that $\bar{r}_d$ can provide evidence for at least 99%
clonal reproduction, but conclude that this inference is dependent on sample 
size and allelic diversity, thus researchers should exercise caution when
