...
population geneticists are able to ask fundamental questions and test hypotheses
...
history violates the fundamental assumption of random assortment of alleles,
...
constrained the researchers to only analyze a minimal set of populations (20) 
per scenario.
...
permutations, it's computationally expensive, and can only take one data set at
...
methods such as Genotyping-By-Sequencing (GBS) and Restriction site associated DNA sequencing (RAD-seq) have rapidly become
...
power analysis of $\bar{r}_d$ to assess sensitivity and specificity, (3) assess
...
Computing Facility (Supplementary information).
...
To calculate the ROC curve, we first define what a positive
value is. If we consider the p-value of $\bar{r}_d$ as a classifier to determine clonality, we can define a positive value as a significant value of $\bar{r}_d$ based on a
threshold, $\alpha$, where $\alpha$ is a value in [0, 1]. Therefore, any
randomly mating population (sexual reproduction is equal to one) with a
significant value of $\bar{r}_d$ is considered a false positive, and
...
data, we decided to ask the question of whether or not using a value of $E_{5A}
\geq 0.85$ could help detect clonal reproduction even if the p-value was 
non-significant. To assess this, we parameterized an additional ROC analysis to
"flip" any non-significant value if it met the above criteria.
...
correction consistently appears to negatively affect the AURC. For all
