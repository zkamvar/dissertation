...
algorithms described in table \@ref(tab:poppr3) [@Agapow:2001]. The user can
...
of your data set (Fig. \@ref(fig:1)). Histograms with rug plots for
...
replications are necessary (Fig. \@ref(fig:2)). *Poppr*
...
(Fig. \@ref(fig:3)) and `poppr.msn()` for any distance matrix. The
...
function `bruvo.boot()` (Fig. \@ref(fig:4)). Neither graphing of
...
```{r, poppr1, results = 'asis', echo = FALSE, fig.cap = cap1, fig.scap = scap1}
knitr::include_graphics("figures/Finland.png")

cap1 <- "Distribution of 12 multilocus genotypes from the `Finland` population 
of the `H3N2` SNP data set [@Jombart:2008]"

cap1 <- beaverdown::render_caption(cap1, figname = "popprfig1", to = TO)
scap1 <- cap1
```

```{r, poppr2, results = 'asis', echo = FALSE, fig.cap = cap2, fig.scap = scap2}
knitr::include_graphics("figures/nancy_5.png")
...
[@Jombart:2008] (a) and for the clonal `Athena` population of the `Aeut` data
set [@Grunwald:2003] (b)."
...
```{r, poppr3, results = 'asis', echo = FALSE, fig.cap = cap3, fig.scap = scap3}
knitr::include_graphics("figures/bruvo_color.png")
...
relatedness. Edge length is arbitrary."
...
```{r, poppr4, results = 'asis', echo = FALSE, fig.cap = cap4, fig.scap = scap4}
knitr::include_graphics("figures/nancy9-boot-redo.png")
...
the `nancycats` data set [@Jombart:2008]"
...
replicates (node values greater than 50% are shown)."
```
...
mutation model appropriate for microsatellite data [@Bruvo:2004]. While this
distance is implemented in the program <span style="font-variant:small-caps;">
GenoDive</span> @genodive and the R package *polysat* @polysat, there are
...
for this distance as shown in Figure \@ref(fig:4).
