...
mutation, 2) genetic drift, 3) gene/genotype flow, 4) mating system, and 5) 
selection due to host resistance [-@mcdonald2002pathogen]. An example of how
changing one of these five factors influences plant health is clear in the
oomycete *Phytophthora infestans* (Mont.) de Bary.

The causal agent of potato late blight and poster child for plant pathology, 
*P. infestans* has changed the very demographics of human populations across
the Atlantic ocean due to its role in the Irish potato famine 
[@erwin1996phytophthora]. It has a heterothallic (no selfing) life cycle that 
includes both clonally and sexually derived infectious propagules (Fig.
\@ref(fig:pinflife)). The clonally derived propagules are swimming short-lived
zoospores that can be transmitted via wind or water, whereas the sexually
derived propagules are double-walled oospores. While *P. infestans* can survive
as hyphae in infected tubers, the sexually derived oospores can survive for
years in the soil, highlighting the added risk of sexual reproduction.
...
type, A1, observed in Europe, meaning that it was reproducing in a strictly
clonal fashion [@mcdonald2002pathogen]. Sexual reproduction became possible when
the second mating type, A2, was introduced. Because of this risk, population
...
could be addressed. Additionally, computational methods were increasingly
important for answering these questions, and as more and more of our analyses 
becoming computational, we need to ensure that our computational methods are
reproducible.

## The Need For Reproducible Research

Computational methods are crucial for population genetic analyses in the 21st
century [@excoffier2006computer]. There have been a call for standards in 
analyzing clonal organisms for the past twenty years, which resulted in a 
plethora of computer programs [@tibayrenc1996towards; @arnaud2007standardizing].
It was not uncommon for a researcher to use several programs to answer a single
research question, and each program would often require its own custom input 
format and would not necessarily run on all operating platforms 
[@excoffier2006computer; @kamvar2014poppr]. The constant reformatting of data,
and the inability to automate series of analyses made it not only challenging to
communicate the methods, but also increased the chance for error 
[@excoffier2006computer; @kamvar2014poppr; @goecks2010galaxy]. 
...
lab notebook, but is not necessarily extended to computational analyses. The
concept of reproducible research in scientific computing is attempting to take
the principles of reproducibility we use in the wet lab and extend them to our
downstream analyses. This work attempts to introduce tools for reproducible 
research in clonal population genetics in an open manner.
