...
US-8, which is polytomic (Table 1).
...
distance (columns) at a threshold of 0.07 with the average
neighbor algorithm [@bruvo2004simple; @sokal1958statistical]. Values in the
table represent the number of times any given inferred MLL matches with a
previously defined MLL. For example, in our original data set, there were three
genotypes previously defined as the US-24 MLL. All three genotypes were also
determined to cluster into a single MLL by filtering. In contrast, US-8 was
determined to cluster into three different MLLs by filtering."
